...
SDLPoP can play Prince of Persia's MIDI music through a **Roland MT-32 / CM-32L emulator**
...
MT-32 would play the music with its wrong, factory instruments (melodies where drums should be),
so SDLPoP treats this file as required for the MT-32 mode and falls back to OPL if it is
missing.
...
2. If that file isn't there, it reads the instruments straight out of **`PRINCE.DAT`**.
    ...
    (the version that introduced Roland support). If your game data comes from an earlier version
    (1.0–1.2), the instruments simply aren't there -- and that's fine. SDLPoP just plays the normal
    OPL/Adlib music instead, which is exactly what those versions always did.
...
* **MT-32** (recommended): `mt32_control.rom` + `mt32_pcm.rom`
* **CM-32L**: `cm32l_control.rom` + `cm32l_pcm.rom`

MT-32 ROMs are used by default because they are the most popular and match the game era. The CM-32L
adds extra PCM samples and may sound slightly different on some sounds -- try it if you prefer its
character. The emulator recognises the ROMs by their contents, so the exact file names only need to
match one of the pairs above (upper- or lower-case both work, but both files must use the same
convention). The `mt32_dac` option is documented below.
...
```
WRITE-LCD: The Princess awaits.
MT-32: Using Roland emulation for MIDI music.
```

when the music starts playing through the emulated Roland. If anything is missing, the game continues
normally with the OPL/Adlib music -- nothing breaks.

Note: There is no console output in the Windows (release) version of the game. But you will definitely
hear the difference in the MIDI music.
...
* `mt32_dac` -- shapes the tone/character of the output, emulating hardware DAC differences.
  ...
  -  `2` -- GENERATION1: emulates the old MT-32 DAC -- warmer/softer top end. Prefers the MT-32 ROM over the CM-32L ROM. (default)
  -  `3` -- GENERATION2: emulates the newer MT-32 / CM-32L DAC.
